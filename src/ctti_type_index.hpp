...
class ctti_type_index: public type_index_facade<ctti_type_index, detail::ctti_data> {
    const char* data_;
    // cutting off end of string_view doesn't work aka via remove_suffix,etc.
    // so had to do this work-around
    char result_str[255];
...
public:
    ...
    BOOST_CXX14_CONSTEXPR inline ctti_type_index() BOOST_NOEXCEPT
        : data_(boost::detail::ctti<void>::n()),result_str{}
    {}

    inline ctti_type_index(const type_info_t& data) BOOST_NOEXCEPT
        : data_(reinterpret_cast<const char*>(&data)),result_str{}
    {}
    ...
    constexpr const char * pretty_name() BOOST_NOEXCEPT;
...
constexpr const char * ctti_type_index::pretty_name()  BOOST_NOEXCEPT 
{
    std::string_view name(raw_name());
   /* std::size_t loc=name.find_last_of("::");
    if (loc != name.npos)
    {
    ...
    {
       std::string_view::iterator it=boost::typeindex::detail::constexpr_search(name.begin(),name.end(),
                                                                        "]","]"+1);
       std::string_view::iterator begin_it=name.begin();

       uint8_t i=0;
       while (begin_it != it)
       {
          result_str[i++]=*begin_it;
          begin_it++;
       }
       result_str[i]='\0';
