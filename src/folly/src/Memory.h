...
/* Kerry Murphy 02/23/2020 added a method inner_allocator to return inner reference to
CxxAllocatorAdaptor allocator */
/* Kerry Murphy 03/01/2020 placed friend functions outside of CxxAllocatorAdaptor so
that it can be inherited or composed from*/
#pragma once
...
bool operator==(CxxAllocatorAdaptor<T, Inner> const& a, 
CxxAllocatorAdaptor<T, Inner> const& b) noexcept
{
...
class CxxAllocatorAdaptor {
 private:
  ...
  //used to hold a default Inner-KM
//  static Inner* defaultInner;

  /*template <typename W,typename Enable=void>
  struct Wrapper;

  // only enabled if W default constructible
  template <typename W>
  struct Wrapper<W,std::enable_if_t<std::is_default_constructible_v<W>>>
  {
     static W inner();
  };*/
 public:
  ...
  //KM used if containers using allocators is buried in another library
  // can't access 
  //explicit CxxAllocatorAdaptor() : ref_(*defaultInner){}
  explicit CxxAllocatorAdaptor(Inner& ref) : ref_(ref) {} 
  template <typename U>
  explicit CxxAllocatorAdaptor(CxxAllocatorAdaptor<U, Inner> const& other)
      : ref_(other.ref_) {}
  /*static void setDefaultInner(Inner & inner)
  {
     defaultInner=std::addressof(inner);
  }*/
  ...
  //template <typename T, typename Inner>
//used to hold a default Inner-KM
  //Inner* CxxAllocatorAdaptor<T,Inner>::defaultInner=nullptr;
