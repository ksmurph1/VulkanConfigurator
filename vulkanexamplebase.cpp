...
void VulkanExampleBase::prepare()
{
	...
	createSynchronizationPrimitives(); // parallel
	setupDepthStencil(); // parallel depends depthFormat
	setupRenderPass(); // parallel depends depthFormat , swapchain.colorFormat
	createPipelineCache(); // parallel
	setupFrameBuffer();
...
bool VulkanExampleBase::initVulkan()
{
	...
	// Vulkan instance
	err = createInstance(ENABLE_VALIDATION);
	if (err) 
	{
	...
	VK_CHECK_RESULT(vkCreateSemaphore(device, &vks::initializers::semaphoreCreateInfo, nullptr, &semaphores.presentComplete));
	// Create a semaphore used to synchronize command submission
	// Ensures that the image is not presented until all commands have been sumbitted and executed
	VK_CHECK_RESULT(vkCreateSemaphore(device, &vks::initializers::semaphoreCreateInfo, nullptr, &semaphores.renderComplete));

	// Set up submit info structure
	// Semaphores will stay the same during application lifetime
	// Command buffer submission info is set by each example

	vks::initializers::submitInfo.pWaitDstStageMask = &submitPipelineStages;
	vks::initializers::submitInfo.waitSemaphoreCount = 1;
	vks::initializers::submitInfo.pWaitSemaphores = &semaphores.presentComplete;
	vks::initializers::submitInfo.signalSemaphoreCount = 1;
	vks::initializers::submitInfo.pSignalSemaphores = &semaphores.renderComplete;
...
void VulkanExampleBase::createCommandPool()
{
	...
	.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT,.queueFamilyIndex = swapChain.queueNodeIndex};
